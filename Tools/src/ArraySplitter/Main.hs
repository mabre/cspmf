...
-- n# must be functions with paramter because the maximum size for static variables
-- in Java is reached after translation to Java. (javac error: code too large -
-- A single method in a Java class may be at most 64KB of bytecode.)
main :: [String] -> IO ()
main args = putStrLn $ show $ unlines $ concat (intersperse " ++ " names) : fs
    where
        name = args !! 0
        arr  = args !! 1
        elems = map atoi $ splitOn ',' arr
...
splitOn sep str = words $ packed $ map (\a -> if a == sep then ' ' else a) str.toList
